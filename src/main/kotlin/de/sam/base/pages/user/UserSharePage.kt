...
//                }
//
                file = FileDAO.findById(ctx.share!!.first.file.id)?.toDTO() ?: throw NotFoundResponse("File not found")
            }
        }
        super.handle(ctx)
    }
    ...
    fun downloadFile(ctx: Context) {
        ...
        val file = transaction {
            FileDAO.findById(ctx.share!!.first.file.id)?.toDTO() ?: throw NotFoundResponse("File not found").also { Logger.error(it.message) }
        }
